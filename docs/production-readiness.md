...
| Workshop profiles pinned from real cuts | **40 %** | kit + admin apply path tested (`test_calibration_admin.py`), provenance + refusal while unresolved | **no real coupon results yet** — all profiles INDUSTRY_TYPICAL_UNCALIBRATED; needs one cut+engraved plate per product×material from the workshop |
| OpenAPI published + canonical layout | **90 %** | `scripts/export_openapi.py --check` in CI; `docs/api/openapi.json` | production `/openapi.json` unreachable from here (deployment down) |
| Customer identity + secure retrieval | **75 %** | `test_customer_identity.py` 3/3, `e2e/customer_identity_e2e.py`; login by contact + code, claim, list, resume with token rotation | no SMS/e-mail provider wired (codes not delivered outside dev echo); no account deletion/export UI |
| Calligraphy sources (Thuluth/Diwani/Ruqaa/Kufi/Nastaliq) | **85 %** | `test_source_registry.py`, `test_font_capability.py`, sweep 1.00 for the new fonts, evidence sheets | Diwani has no rights-cleared source (upload held out); Thuluth heavy strokes rely on the realism normaliser |
...
`cd backend && python3 -m pytest -q` on HEAD `8c4bf41` (PostgreSQL 16, 4 cores): **511 passed, 0 failed, exit 0** in ~35 min (log `full_s5.log`, 2026-09-05 15:05–15:41 UTC). Re-run on `9fd2379` (approval link + PNG export): 515 collected, 513 passed; the 2 failures were the tests that encoded "png is never an export" — updated in `3c513de` to the real contract (AI images never exported; PNG = lock-gated raster of the master vector, kind=preview) and passing, i.e. effective **515/515** on the committed tree. **2026-09-10, HEAD `1367218`** (fonts, script priority, shared limiter/storage, customer identity, OpenAPI gate, calibration API): **526 passed, 0 failed, exit 0** (`full_s8.log`). An earlier run on the pre-fix tree timed out at 50 min because the seven-name generation was 75 s per call under CPU contention; that is what the perf work in `8c4bf41` addressed. CI: runs 53–56 failed only on the three tests fixed in `8c4bf41` (phrase routing ×2, vector-edit text protection); run 57 (`8c4bf41`) passed the backend suite but failed the evidence step (seven-name family count, fixed in `5686ce3`); **runs 59 (33976307374), 60 (33976362966), 61 (33977253484, secure approval link) and 62 (33977527995, proof fix) are green on all four jobs** — backend suite, evidence report, frontend build, browser E2E incl. `approval_link_e2e.py`. Run 63 (`9fd2379`, PNG export) failed on the two tests that encoded "png is never an export"; they now assert the real contract (AI images are never exports; the PNG is a lock-gated raster of the master vector, kind=preview) — **runs 64 (33980721633, `3c513de`) and 65 (33980737422, `48c6e9d`) are green on all four jobs**, which made run 65 the reference CI run for the 2026-09-05 state. **2026-09-10: runs 67–70 (34426458839, 34427319432, 34428047159, 34428059280 on HEAD `1367218`) are green on all four jobs; run 70 is the reference run for this report.** **Owner-sample / specification slices (2026-09-10 pm): local captured full suite on `df3eb8c` — 544 passed, 0 failed, pytest exit 0; CI run 73 (`7b73f13`) and 74 (`b64cd74`) green on all four jobs; runs 75–76 failed only on `test_golden_case_pattern_is_flagged_where_relevant` (cufflinks now have a golden case), fixed in `df3eb8c`; run 78 (34476650825, `df3eb8c`) green on all four jobs — reference run for these slices.**
...
### Owner sample ingestion (2026-09-10 — 112 images in seven batches)
| Capability | Status | Evidence | Honest limitation |
|---|---|---|---|
| 36 owner cases (BS-GPC-0003…0040) as structured construction lessons | WORKING | `test_golden_production_memory.py` (17 tests, 38 cases seed idempotently); `docs/reference/owner-samples/README.md` index | No text read off photos: every owner case is `customer_source_text=None`, pending tier; lessons are human-curated, not measured |
...
| Privacy: children / faces / on-body / customer name+date photos hash-only | WORKING | `test_third_party_and_personal_evidence_is_hash_only` (60 `EXCLUDED_*` items, none exported) | Owner-owned binaries still `PENDING_OBJECT_STORE_UPLOAD` (only downscaled docs copies exist); the retail-display survey (stock name necklaces, engraved bars, cord bracelets photographed in a shop) is UNKNOWN_RIGHTS — lessons only |
...
| Jewellery specification knowledge → copilot expert brief | WORKING (online research SKIPPED_NETWORK_BLOCKED) | `app/data/jewellery_specifications.json` (11 evidence-labelled sections), `services/specifications.py`, `retrieve_design_memory(product=…)` → `expert_brief`, `GET /api/products/specifications`; `tests/test_specifications.py` (6); `docs/evidence/specification_sources.json` (0 reachable / 11 blocked) | Trade values are unverified online (egress blocked); verifier + human confirmation needed before quoting a value as a standard; UAE hallmark rule flagged for confirmation |
