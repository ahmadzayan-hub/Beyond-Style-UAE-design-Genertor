...
- VERIFIED_LOCAL — privacy/rights: 36 evidence items are hash-only (`EXCLUDED_PERSONAL_DATA` for children/faces/on-body/customer name+date; new `EXCLUDED_THIRD_PARTY_RIGHTS` for other brands / unknown origin); `rights_provenance` THIRD_PARTY_NO_COPY / UNKNOWN_RIGHTS cases are excluded from training export and from `golden_case_influence`. Only owner-owned, people-free images are copied (downscaled) to `docs/reference/owner-samples/` (45 files + README index).
