...
## Backend go-live in progress on Replit (2026-09-15 evening, owner "go ahead")
Monitor run 64 (`60d5946`, 19:12–19:13 UTC) is the first run in which the Replit subdomain served THIS backend: `/ready` 200, Arabic 7-name design created with the text preserved byte-for-byte, 10 candidates, selection, manufacturing validation, AI status — 13/15 checks. The two misses were the very first `/health` call (no response within 15 s: autoscale cold start; the monitor now retries three times with a 25 s timeout) and the CORS preflight (no `Access-Control-Allow-Origin` in the answer — the monitor now prints the preflight status and headers so the cause is visible). `/health` did not carry `build_sha`, so the running build predates the sync commit `676f24f` or was built without git. Run 65 (`fd99732`, 19:15 UTC), two minutes later, got the OTHER Arabic HTML app on the same subdomain again — the subdomain is flipping between two deployments, i.e. a rollout/replace is still in progress on Replit. The `workflow_dispatch` trigger added in `60d5946` cannot be fired by this session's GitHub integration (403), so re-probes happen on push or via the **Run workflow** button in GitHub → Actions → Production Monitor.

## Correction — local full-suite claims for 938497d / dbe9d1e (2026-09-10)
The two "full local backend suite: exit 0" statements in the commit messages of `938497d` and `dbe9d1e` were NOT valid evidence: the suite was run as `pytest … | tail`, so the recorded exit code was `tail`'s, and PostgreSQL had stopped in the container during the second run. CI run 75 (`938497d`) and run 76 (`dbe9d1e`) failed on `tests/test_human_review.py::test_golden_case_pattern_is_flagged_where_relevant`, which still asserted that cufflinks have no golden case — they now have one (BS-GPC-0007 manufactured enamel cufflinks, plus the relief-disc line). The test now asserts the manufactured cufflinks case leads and uses a product no case covers for the empty check. Full-suite evidence below is only quoted from runs whose pytest exit code was captured directly.
**Captured run on `df3eb8c` (2026-09-10, PostgreSQL 16, local): 544 tests, 0 failures, 0 errors, pytest exit 0** (`scratchpad/full_suite3.log`; the summary line is suppressed because pytest.ini already sets `-q`, so the count is taken from the progress dots: 544 `.` / 0 `F` / 0 `E`). CI run 74 (`b64cd74`) green on all four jobs; runs 75–76 red only on the cufflink review-pattern test fixed in `df3eb8c`; run 77 (`53934e7`) red on the same pre-fix test; **run 78 (34476650825, `df3eb8c`) green on all four jobs — the reference CI run for the owner-sample / specification slices**; run 79 (`6bbe1ec`, docs only) in progress.

## Jewellery specification knowledge for the design copilot (2026-09-10, owner: "search online and learn the jewellery specifications")
- SKIPPED_NETWORK_BLOCKED — online research could not run here: WebSearch returned unavailable and WebFetch to en.wikipedia.org, ganoksin.com and moiat.gov.ae was refused by the egress proxy (EGRESS_BLOCKED). `scripts/verify_specification_sources.py` re-probes every external `verify_at` URL and wrote `docs/evidence/specification_sources.json`: 0 reachable, 11 blocked/unreachable, 9 in-repo references. Nothing is presented as online-verified.
- VERIFIED_LOCAL — `backend/app/data/jewellery_specifications.json` (11 sections: ring sizing ISO 8653 table + band rules, sheet thickness/gauge + cut-out limits, findings/attachments, chains, stones/pearls, enamel, metal fineness/density/marking, product size envelopes, engraving, Arabic-text-on-jewellery rules, customer-request playbook). Every section carries an evidence level (IN_REPO_VERIFIED / OWNER_STATED / TRADE_STANDARD_UNVERIFIED_ONLINE) and `verify_at` sources; `test_trade_values_agree_with_enforced_materials_limits` proves it never promises softer limits than materials.json.
- VERIFIED_LOCAL — `app/services/specifications.py`: `sections_for_product`, `size_envelope(product, audience)`, `expert_brief(product, audience, material, text_length)` = trade rules + proven Golden-Memory lessons (owned cases only) + situational notes, always stating that deterministic rules override it.
- VERIFIED_LOCAL — copilot tool `retrieve_design_memory` accepts optional `product/audience/material/text_length` and returns `expert_brief` (existing keys unchanged; `test_hermes_tools_live` + `test_external_ai_acceptance` still green); `GET /api/products/specifications[?product=…]` serves the same (OpenAPI 95 paths). `tests/test_specifications.py`: 6 tests.
- Honest gaps: trade values are written from established practice, not fetched; the UAE hallmarking rule is flagged "confirm with MoIAT / Dubai Central Laboratory"; raising any evidence level requires a person to confirm the values against the listed source after the verifier reports it REACHABLE.

## Owner sample ingestion → Golden Production Memory + product catalogue (2026-09-10)
Owner supplied 112 real images in seven batches ("real samples to LLM to learn"). Ingested honestly:
- VERIFIED_LOCAL — 36 new golden cases (BS-GPC-0003…0040) in `backend/app/data/golden_production_cases.py`: bar-name pendant (layout proof + workshop outline + product), pavé name necklace, Latin script bracelet, open name ring, enamel cufflinks, engraved disc keychains, pierced name disc, orchid enamel brooch (vector + product), Arabic name bar-pin brooch, lariat letter stations, car mirror hanger, name bracelet/necklace lines, intertwined calligraphy pendant, phrase plate on mesh band, men's chain catalogue, kids' and cufflink market references, length guides, a retail-display survey (stock name necklaces / engraved bars / cord bracelets, UNKNOWN_RIGHTS), the packaging board, relief cufflinks creative, a letter-station bead necklace, vendor font tables / font comparison sheets (unidentified commercial fonts — nothing traced), a framed calligraphy pendant, phrase-library screenshots (one Qur'anic verse → sacred-text path), and a car-hanger creative with a car-brand medallion recorded as a brand-risk example, a Top Style retail stock survey (third-party carded stock incl. a pavé sacred word) and supplier renders of a leather-strap phrase plate / rope cuff with pavé name / pavé halo frame (unknown rights), and the brand mood board (AI-composited faces, not copied); the finished gold + silver mesh bracelets upgraded BS-GPC-0025 to MANUFACTURED_OWNER_SAMPLE. 38 cases seed idempotently; 17 DB tests pass.
...
- VERIFIED_LOCAL — privacy/rights: 60 evidence items are hash-only (`EXCLUDED_PERSONAL_DATA` for children/faces/on-body/customer name+date; new `EXCLUDED_THIRD_PARTY_RIGHTS` for other brands / unknown origin); `rights_provenance` THIRD_PARTY_NO_COPY / UNKNOWN_RIGHTS cases are excluded from training export and from `golden_case_influence`. Only owner-owned, people-free images are copied (downscaled) to `docs/reference/owner-samples/` (51 files + README index).
...
**2026-09-10 13:55 UTC — Vercel check (owner: "run the project on vercel").** Vercel project `frontend` (`prj_qf9LfOdeVRzfYTZ39sS6pja9iDCm`, team `celia2026-3923s-projects`, Hobby plan) is linked to this repository and auto-deploys pushes; the latest production deployment `dpl_F2YVh5tXi6Fvhsw3nJKCpEseebjJ` was built from `6ab90e0` on `claude/p0-golden-path-audit-jtyduw` (target=production, READY in 30 s, region iad1). `https://frontend-sigma-sable-22.vercel.app/` returns 200 with the Golden Path page (RTL Arabic UI, security headers, CSP `connect-src` = the Replit backend origin); no runtime errors in the last 24 h. **Access:** Vercel Authentication (SSO) shows as enabled (all_except_custom_domains) on the project, but the production alias is publicly reachable: the Production Monitor's anonymous GitHub runner gets HTTP 200 on `https://frontend-sigma-sable-22.vercel.app/` (check A) on every run, so customers are not blocked by it. An attempt to switch the setting off through the Vercel connector returned 403 (the connected Vercel account `celia2026-3923` cannot update the project) — nothing to change there anyway. **Backend — DOWN (owner report 2026-09-10 ≈14:00 UTC: "تعذر الاتصال بالخادم" on the live site).** The Production Monitor workflow (probes the real URLs from GitHub's network on every push) has failed on **all 53 runs since 2026-09-02**: check A (frontend loads) passes, check B fails with "URL answers 200 but is NOT the Beyond Style backend" — `https://beyond-style-uae-design-genertor.replit.app/health` returns a 200 page that is not this FastAPI app (the smoke script's own comment records an AI-scaffolded SPA occupying that Replit subdomain). So the frontend is live but its API origin is not serving this backend; every customer action fails at the first request. **Nothing in this repo can fix that** — it needs the owner on Replit: publish THIS repository's backend (`.replit` runs `alembic upgrade head` + uvicorn) as the deployment at that subdomain (or a new one), set `DATABASE_URL`, `ALLOWED_ORIGINS=https://frontend-sigma-sable-22.vercel.app`, `OBJECT_STORAGE=s3`, `SECURE_HSTS=true`, then confirm `/health` returns JSON with `status: ok` and `schema_version`; if the URL changes, set Vercel `NEXT_PUBLIC_API_URL` and `deploy/production.json`. **GitHub ↔ Replit sync (owner ask, 2026-09-10):** GitHub is the source of truth and this repository has a single default branch, so a Replit import gets exactly the code Vercel builds; Replit never pulls or redeploys by itself. Measured sync: the `.replit` deployment build now records the checked-out commit in `backend/app/BUILD_SHA` (git-ignored), `/health` reports it as `build_sha` (`unknown` when not recorded — never invented; `test_health_reports_the_running_commit_honestly`), and the Production Monitor prints `B2. sync — IN_SYNC / BEHIND: deployed <sha> vs pushed <sha>` using `GITHUB_SHA`. Procedure in `replit.md` (Git → Pull, Deployments → Redeploy). CI runs 82 (`446efe0`), 83 (`0d68461`) and **84 (34518545564, `676f24f`) green on all four jobs**; monitor run 56 executed the new smoke script cleanly and still stops at check B until the backend is published from this repository. Monitor run 54 (`446efe0`, 14:15 UTC) shows what the subdomain serves: an Arabic RTL HTML page (`<!DOCTYPE html> <html lang="ar" dir="rtl">…`) — a different web app is deployed at that Replit address, not this FastAPI backend. The frontend's connection error no longer blames the customer's internet; it says the Beyond Style server is unavailable.
