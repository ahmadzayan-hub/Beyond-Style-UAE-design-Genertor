...
## PRODUCTION RELEASE HARDENING (this slice, see RELEASE_EVIDENCE.md)
**FINAL DECISION: NOT PRODUCTION READY.** Full evidence + exact blocker/owner/action table in `RELEASE_EVIDENCE.md` at repo root.
- **Next.js 14.2.13 → 15.5.24** (closes the reachable advisories behind npm audit's Critical+High rollup — this app has no Server Actions/`next/image`/middleware, but `rewrites()` is always compiled into the production build and its request-smuggling advisory is now fixed) + project's own postcss 8.4.47 → 8.5.26. Clean typecheck/build, 5/5 E2E.
- **FastAPI 0.115.0 → 0.135.0 + starlette 1.3.1 attempted, then REVERTED**: passed all 230 unit tests (TestClient) but broke real uvicorn HTTP request handling — `POST /candidates` looked fine but the rows were never visible to a subsequent request (`GET .../svg` 404, `GET .../candidates` → `[]`), 100% reproducible via curl, not reproducible in-process. Reverted to `fastapi==0.115.0`; re-verified 230/230 + 5/5 E2E + 15/15 smoke green again. **The 9 starlette CVEs remain open** — this was a real, deliberate decision to not ship a P0 break for a security fix, not an oversight.
- pytest 8.3.3 → 9.0.3 (dev-only, not reachable in production, bumped anyway) → **0 known vulnerabilities in `backend/requirements.txt`** (pip-audit), down from python-multipart/pillow/fonttools findings fixed in the prior slice.
- **Real GitHub Actions CI root-cause found**: all 9 prior CI runs failed; run 10's actual test suite passed but the job was marked failed by a GitHub-side "Artifact storage quota has been hit" error on the evidence-upload step — unrelated to code. Fixed: `continue-on-error: true` on both `upload-artifact` steps in `ci.yml`.
- **Vercel SSO deliberately left enabled** per this slice's explicit instruction to keep it enabled until security blockers clear — still true (starlette CVEs open) — even though `update_project_deployment_protection` (a real tool) could disable it.
- **Still blocked, not assumed complete**: backend never deployed anywhere (no Replit access), `NEXT_PUBLIC_API_URL` unset on Vercel (no env-var-write tool), `beyondstyle.ae` custom domain not attached. Production smoke/E2E/7-name-scenario-through-the-live-UI remain unrun against real URLs for this reason.
