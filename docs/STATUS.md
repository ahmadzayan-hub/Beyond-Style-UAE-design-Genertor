...
## P1 PRODUCTION DEPLOYMENT CLOSURE (this slice, see RELEASE_EVIDENCE.md)
**FINAL DECISION: NOT PRODUCTION READY.** Full evidence + exact blocker/owner/action table in `RELEASE_EVIDENCE.md` at repo root.
- **Real, previously-undiscovered CI-blocking bug found and fixed**: `python-multipart` (required by FastAPI's `Form()`/`File()` multipart parsing — the reference-image upload endpoint) was never pinned in `backend/requirements.txt`. It happened to be pre-installed as a stray package in this sandbox, masking the gap; pulled real GitHub Actions job logs and confirmed this is why **all 9 prior CI runs on this branch failed** at test-collection time. Fixed (`python-multipart==0.0.31`) and verified 230/230 in a from-scratch clean venv (twice).
- **Real dependency security scan run** (`pip-audit`/`npm audit`, not previously done): fixed Pillow (24 CVEs, incl. heap out-of-bounds writes reachable via uploaded images) and fonttools (CVE-2025-66034) — both verified against the full suite in a clean venv. Found and left **open** (reported, not force-fixed): Next.js has 1 unresolved Critical + 1 High finding requiring a 2-major-version upgrade; Starlette has 9 unresolved CVEs blocked by FastAPI's `starlette<0.39.0` pin — both need a dedicated upgrade+regression slice, out of scope for a safe deployment-closure pass.
- **Real production-discovery finding**: the one Vercel deployment that exists (`dpl_6aMGbMLjF1Z3yCexXRQfn6tEGJEE`, commit-exact match, READY) is **not publicly reachable** — Vercel Authentication (SSO) is enabled project-wide except for a custom domain, and no custom domain is configured, so every existing URL requires a Vercel login. Separately, this session's own network egress is policy-blocked to `vercel.app` (confirmed via the agent-proxy status endpoint), so this session could not load the production URL itself even to check.
- **Still blocked, not assumed complete**: backend has never been deployed anywhere (no Replit access in this session) and `NEXT_PUBLIC_API_URL` is still unset on Vercel (no env-var-write tool available). Production smoke/E2E/7-name-scenario-through-the-live-UI remain unrun against real URLs for this reason.
