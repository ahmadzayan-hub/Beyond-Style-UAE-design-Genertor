...
| 4 | ~~Starlette has 9 unresolved CVEs~~ **RESOLVED 2026-08-29**: fastapi 0.141.1 + starlette 1.6.0 pinned; the regression was a commit-after-response race (see docs/STATUS.md security slice) | Done | The smallest compatible fix (`fastapi==0.135.0`+`starlette==1.3.1`) was tried and caused a real P0 regression (candidate persistence invisible under real HTTP handling) — reverted, not re-attempted this slice per explicit instruction. Needs a proper bisection/reproduction outside this app before retrying. |
